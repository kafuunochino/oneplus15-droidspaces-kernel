...
readonly NEW_VARIANT="${NEW_VARIANT:-kabi-pointer-state}"
...
split = section("split")
for key in ("matched", "mismatched", "missing_from_gki", "modules_with_mismatch"):
    if counter(pointer, key) != counter(split, key):
        raise SystemExit(f"pointer/split counter differs: {key}")
...
print("Verified: pointer-state CRCs are identical to split baseline")
...
grep -qx '# CONFIG_USER_NS is not set' "$NEW/kernel.config"

echo 'All pointer-state offline ABI gates passed.'
