#!/usr/bin/env bash
# Package the gaming test Image into the audited stock boot layout.
set -Eeuo pipefail
...
readonly FLAVOR="${FLAVOR:-gki-r53-gaming-userns-ntsync}"
readonly FINAL_NAME="${FINAL_NAME:-final-gki-r53-gaming-userns-ntsync-test}"
readonly MIDAS_GUARD="${MIDAS_GUARD:-1}"
...
readonly AUDIT="$ROOT/SOURCE-AUDIT-GAMING.md"
readonly GAMING_PATCH="$ROOT/github-ci/patches/013.enable-gaming-userns-ntsync.patch"
readonly REPRO="$FINAL/reproducible-build"
...
cp "$AUDIT" "$FINAL/SOURCE-AUDIT.md"
cp "$GAMING_PATCH" "$REPRO/013.enable-gaming-userns-ntsync.patch"
cp "$ROOT/scripts/server-build-gki-r53-gaming.sh" "$REPRO/"
cp "$ROOT/scripts/server-validate-gki-r53-gaming.sh" "$REPRO/"
cp "$ROOT/scripts/server-package-gki-r53-gaming.sh" "$REPRO/"
...
- 8900 common exports have zero non-Rust CRC changes versus the split baseline.
- All changed export CRCs are Rust-only; zero non-Rust changes were found.
- 538 normal stock modules have no added CRC or missing-symbol incompatibility.
- The unused system_dlkm/rust_binder.ko retains its four known Rust CRC
  mismatches and, with USER_NS enabled, also lacks rust_helper_from_kuid.
- CONFIG_USER_NS=y is enabled for Steam Runtime / pressure-vessel.
- CONFIG_NTSYNC=y uses the NT synchronization implementation already present
  in the exact stock GKI r53 source. No third-party executable code is added.
$midas_note

This is a separate gaming TEST image. USER_NS intentionally relaxes the stock
hardening policy. Boot success and offline ABI checks do not prove that every
Steam/Proton game is compatible. Keep the known-good daily boot and stock boot
images plus the fastboot rollback command ready.
EOF

cat > "$FINAL/FLASHING.md" <<EOF
# Manual test and rollback

This image is only for OnePlus 15 PLK110 running OxygenOS
PLK110_16.0.9.400(CN01), with an unlocked bootloader. It is a TEST build.

Before flashing, keep the matching original boot image and the known-good
Droidspaces daily boot on the computer. Confirm the current slot:

    fastboot getvar current-slot

Prefer a temporary boot first when the bootloader supports it:

    fastboot boot $boot_name

If temporary boot is unsupported, flash only the current boot partition:

    fastboot flash boot $boot_name
    fastboot reboot

Do not flash vendor_boot, init_boot, dtbo, or vbmeta for this test.

After Android starts, test Wi-Fi, Bluetooth, mobile data, audio, camera,
fingerprint, charging, DRM/TEE-dependent apps, and SukiSU before starting a
Droidspaces container. Then check:

    adb shell uname -r
    adb shell su -c "/data/local/Droidspaces/bin/droidspaces check"
    adb shell su -c "zcat /proc/config.gz | grep -E 'CONFIG_(USER_NS|NTSYNC|SYSVIPC|POSIX_MQUEUE|PID_NS|IPC_NS|DEVTMPFS)='"
    adb shell su -c "ls -l /dev/ntsync"

If Android does not boot or any hardware function fails, return to fastboot
and flash the matching original boot image:

    fastboot flash boot boot_a.img
    fastboot reboot

The filename boot_a.img above is an example: use the original image for the
slot reported by fastboot. Never use an image from a different OxygenOS build.
EOF
...
hash_files=(
	...
	test-avb-info.txt
	reproducible-build/013.enable-gaming-userns-ntsync.patch
	reproducible-build/server-build-gki-r53-gaming.sh
	reproducible-build/server-validate-gki-r53-gaming.sh
	reproducible-build/server-package-gki-r53-gaming.sh
)
if [[ "$MIDAS_GUARD" == 1 ]]; then
